    ...
    if connection.is_connected():
        for i in range(30):
            ...
            if (((str(result).split('(')[1]).split(',')[0])=="None" or  ((str(result).split('(')[1]).split(',')[0])==""):
                ...
                result_day_out=int((str(result).split('(')[1]).split(',')[0])//int(pcs_div)
...
def result_day_out(qty_day=None):
    ...
    if connection.is_connected():
        ...
        my_sql_select_query_mid = """ SELECT max(quantity) FROM khoytotal WHERE date_stamp LIKE %s AND CID=20 ;"""
